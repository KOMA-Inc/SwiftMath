...
func getInterElementSpaceArrayIndexForType(_ type: MTMathAtomType, row: Bool) -> Int {
    switch type {
    case .color, .textcolor, .colorBox, .boxed, .ordinary, .placeholder:   // A placeholder is treated as ordinary
        return 0
    case .largeOperator:
        return 1
    case .binaryOperator:
...
class MTTypesetter {
    ...
    var currentLineIndexRange = NSRange(location: 0, length: 0)
    ...
    static func createLineForMathList(_ mathList: MTMathList?, font: MTFont?, style: MTLineStyle, cramped: Bool, spaced: Bool) -> MTMathListDisplay? {
        ...
        let last = lastAtom?.indexRange ?? NSRange(location: 0, length: 0)
        let line = MTMathListDisplay(withDisplays: typesetter.displayAtoms, range: NSRange(location: 0, length: NSMaxRange(last)))
        return line
    }
    ...
    init(withFont font: MTFont?, style: MTLineStyle, cramped: Bool, spaced: Bool) {
        ...
        self.style = style
        self.currentLineIndexRange = NSRange(location: NSNotFound, length: NSNotFound)
    }
    ...
    func addDisplayLine() -> MTCTLineDisplay? {
        // add the font
        currentLine.addAttribute(kCTFontAttributeName as NSAttributedString.Key, value: styleFont.ctFont as Any, range: NSRange(location: 0, length: currentLine.length))
    ...
    func makeScripts(_ atom: MTMathAtom?, display: MTDisplay?, index: UInt, delta: CGFloat) {
        ...
        if atom!.superScript == nil {
            ...
            // add the subscript
            _subscript?.position = CGPoint(x: currentPosition.x, y: currentPosition.y - subscriptShiftDown)
        ...
        if atom!.subScript == nil {
            superScript!.position = CGPoint(x: currentPosition.x, y: currentPosition.y + superScriptShiftUp)
        ...
        superScript?.position = CGPoint(x: currentPosition.x + delta, y: currentPosition.y + superScriptShiftUp)
        displayAtoms.append(superScript!)
        ssubscript?.position = CGPoint(x: currentPosition.x, y: currentPosition.y - subscriptShiftDown)
    ...
    func getRadicalGlyphWithHeight(_ radicalHeight: CGFloat) -> MTDisplayDS? {
        ...
        if glyphDisplay == nil {
            ...
            glyphDisplay = MTGlyphDisplay(withGlpyh: glyph, range: NSRange(location: NSNotFound, length: 0), font: styleFont)
    ...
    func makeLargeOp(_ op: MTLargeOperator!) -> MTDisplay? {
        ...
        if op.nucleus.count == 1 {
            ...
            // add the font
            line.addAttribute(kCTFontAttributeName as NSAttributedString.Key, value: styleFont.ctFont!, range: NSRange(location: 0, length: line.length))
